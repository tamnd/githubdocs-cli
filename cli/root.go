// Package cli builds the ghdocs command tree on top of the githubdocs library.
package cli
...
type App struct {
	client *githubdocs.Client
	cfg    githubdocs.Config
...
func Root() *cobra.Command {
	app := &App{cfg: githubdocs.DefaultConfig()}

	root := &cobra.Command{
		Use:   "ghdocs",
		Short: "Browse GitHub documentation",
		Long: `ghdocs reads GitHub Docs through the public search API.
It returns records as table, JSON, JSONL, CSV, TSV, or URLs.

ghdocs is an independent tool and is not affiliated with GitHub.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	pf.StringVar(&app.output, "output", "auto", "output: table|json|jsonl|csv|tsv|url|raw (auto=table on TTY, jsonl piped)")
	...
	pf.StringVar(&app.cfg.BaseURL, "base-url", app.cfg.BaseURL, "GitHub Docs base URL")
...
func (a *App) setup() error {
	...
	a.client = githubdocs.NewClient(a.cfg)
	return nil
}
