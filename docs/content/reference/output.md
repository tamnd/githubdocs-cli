...
describes what users get. Pick a format with `-o`, or let ghdocs choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ghdocs <command> -o table   # aligned columns for reading
ghdocs <command> -o jsonl   # one JSON object per line, for piping
ghdocs <command> -o json    # a single JSON array
ghdocs <command> -o csv     # spreadsheet friendly
ghdocs <command> -o tsv     # tab-separated
ghdocs <command> -o url     # just the URL column
ghdocs <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ghdocs <command> --fields id,title,url
```
...
```bash
ghdocs <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ghdocs <command>            # a table, because this is a terminal
ghdocs <command> | wc -l    # JSONL, because this is a pipe
```
